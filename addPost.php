...
$postname=$_FILES['postimage']['name'];
$posttmp=addslashes(file_get_contents($postname));
...
$qry = "INSERT INTO `post`(username,posttmp,postname,hashtag,date,download) VALUES('$username','$posttmp','$postname','$hashtag','$datetmp','$k')";
...
if(mysqli_query($con,$qry))
{
			...
			preg_match_all('/#([^\s]+)/', $hashtag, $matches);
			//print_r($matches);

			$hash = implode(',', $matches[1]);
			$myArray = explode(',', $hash);
			foreach($myArray as $my_Array){
				$qry3 = "INSERT INTO `searchtag`(hashtag,count) VALUES('$my_Array','$k')";
    ...
    echo '<script>alert("Post added!!!");location="UserHome.php";</script>';
}
else {
    echo '<script>alert("You Did not have permission to write a blog!!!");</script>';
}
	//echo "Inserted!";
}else{
	//echo '<script>alert("You did not have permission to write a blog!!!");location="UserHome.php";</script>';
...
<div class="container">
  ...
  <form class="form-horizontal" method="post" enctype="multipart/form-data" >
    ...
    <div class="form-group">
      <label class="control-label col-sm-2" for="Post Image">Image :</label>
      <div class="col-sm-10">
	...
	<div class="form-group">
      <label class="control-label col-sm-2" for="hashtag">Captian :</label>
      <div class="col-sm-10">
        <input type="text" class="form-control" id="hashtag" placeholder="Captian" name="hashtag">
      </div>
    </div>
